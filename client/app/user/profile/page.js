...
const page = () => {
  ...
  return (
    <form className='mt-5 flex flex-col max-w-3xl mx-auto w-full items-center mb-20'>
      <h1 className='text-zinc-600 md:text-xl text-left w-full'>Profile Settings</h1>

      <div className="mt-5 p-5 space-y-5 w-full bg-slate-50 shadow-lg shadow-black/10 border border-zinc-200 rounded-xl">
        <h1 className='text-zinc-400 text-left w-full'>Profile Details</h1>
        <div className="grid md:grid-cols-2 grid-cols-1 md:gap-5 gap-3">
          <div className="flex flex-col gap-1">
            <label className='text-zinc-400/80'>Full Name</label>
            <input value={user?.name || ''} readOnly className="cursor-not-allowed opacity-60 border border-zinc-300 hover:border-zinc-500 transition-all rounded w-full p-2 text-zinc-800" />
          </div>
      ...
      <DeleteAccount deleteModal={deleteModal} setDeleteModal={setDeleteModal} userId={user?.userId} />
    </form>
  )
}
