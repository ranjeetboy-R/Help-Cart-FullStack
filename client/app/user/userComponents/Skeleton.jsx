...
export const HomePageSkeleton = () => {
    return (
        <div className="md:max-w-lg mx-auto bg-white backdrop-blur-2xl z-50 w-full h-full absolute top-0 left-1/2 -translate-x-1/2 mt-10 flex flex-col gap-3">

            {
                Array(4).fill(0).map((_, index) => (
                    <div key={index} className="h-33 rounded-xl bg-white shadow border border-slate-100 flex p-3 gap-5">
                        <div className="aspect-square skeleton rounded-2xl"></div>
                        <div className="flex-1 flex flex-col justify-center gap-2">
                            <div className="w-45 h-8 rounded-xl skeleton"></div>
                            <div className="w-38 h-6 rounded-xl skeleton"></div>
                            <div className="w-38 h-6 rounded-xl skeleton"></div>
                        </div>
                        <div className="flex flex-col justify-center gap-2">
                            <div className="w-7 h-7 skeleton rounded-full"></div>
                            <div className="w-7 h-7 skeleton rounded-full"></div>
                            <div className="w-7 h-7 skeleton rounded-full"></div>
                        </div>
                    </div>
